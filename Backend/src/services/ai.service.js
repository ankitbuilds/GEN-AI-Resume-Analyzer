    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            doc.fontSize(18).font("Helvetica-Bold").fillColor("#000000")
               .text(parsed.name || "Professional Resume", { align: "center" })
            doc.moveDown(0.2)

            // Contact info line
            if (parsed.contactLine) {
                doc.fontSize(8).font("Helvetica").fillColor("#333333")
                   .text(parsed.contactLine, { align: "center" })
                doc.moveDown(0.5)
            }
            ...
            for (const sectionName of sectionOrder) {
                ...
                drawSectionHeader(doc, sectionName, pageWidth, leftMargin)

                if (sectionName === "Technical Skills") {
                    renderSkillsSection(doc, section, leftMargin, pageWidth)
                } else if (sectionName === "Education") {
                    renderEducationSection(doc, section, leftMargin, pageWidth)
                } else if (sectionName === "Professional Experience") {
                    renderExperienceSection(doc, section, leftMargin, pageWidth)
                } else if (sectionName === "Projects") {
                    renderProjectsSection(doc, section, leftMargin, pageWidth)
            ...
            // Render any remaining unparsed sections
...
function parseResumeText(resumeText, selfDescription) {
    ...
    for (let i = 0; i < Math.min(lines.length, 5); i++) {
        ...
        if (!line) continue
        // Name: short line, not a section header, not contact info
        if (line.length < 60 && !isSectionHeader(line) && !isContactInfo(line)) {
            result.name = line
            startIdx = i + 1
            break
        }
    ...
    // Extract contact info (lines with phone/email/linkedin/github near the top)
    const contactParts = []
    for (let i = startIdx; i < Math.min(lines.length, startIdx + 4); i++) {
        const line = lines[i].trim()
        if (!line) continue
        if (isContactInfo(line)) {
    ...
    const normalizeSection = (name) => {
        ...
        if (lower.includes("technical skill") || lower === "skills") return "Technical Skills"
        if (lower.includes("experience") || lower.includes("work")) return "Professional Experience"
        if (lower.includes("certification") || lower.includes("training")) return "Certifications"
    ...
    for (let i = startIdx; i < lines.length; i++) {
        ...
        if (isSectionHeader(trimmed)) {
            ...
            if (currentSection && currentContent.length > 0) {
                const normalized = normalizeSection(currentSection)
                if (!result.sections[normalized]) result.sections[normalized] = []
...
function isContactInfo(line) {
    return /(@|gmail|yahoo|hotmail|outlook|linkedin|github|phone|mobile|\+\d{1,3}[\s-]?\d|\.com)/i.test(line)
}
...
function drawSectionHeader(doc, title, pageWidth, leftMargin) {
...
function renderSkillsSection(doc, lines, leftMargin, pageWidth) {
    for (const line of lines) {
        checkPageBreak(doc, 20)
        // Detect "Category: value" pattern
        ...
        if (colonIdx > 0 && colonIdx < 40) {
            ...
            doc.text(label, leftMargin, doc.y, { continued: true })
            doc.font("Helvetica").text(" " + value)
        } else {
            doc.fontSize(9.5).font("Helvetica").fillColor("#000000")
               .text(line, leftMargin)
        }
    }
    doc.moveDown(0.3)
}

function renderEducationSection(doc, lines, leftMargin, pageWidth) {
    // Try to detect institution + date pairs
    let i = 0
    while (i < lines.length) {
        const line = lines[i]
        checkPageBreak(doc, 40)

        // Check if line contains a date pattern on the right
        const dateMatch = line.match(/((?:January|February|March|April|May|June|July|August|September|October|November|December|Jan|Feb|Mar|Apr|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[\s.]*\d{4}(?:\s*(?:to|-|–)\s*(?:Present|(?:January|February|March|April|May|June|July|August|September|October|November|December|Jan|Feb|Mar|Apr|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[\s.]*\d{4}))?|\d{4}\s*(?:to|-|–)\s*(?:Present|\d{4}))/i)

        if (dateMatch) {
            const dateText = dateMatch[0]
            const mainText = line.replace(dateText, "").trim()

            // Render institution name bold left, date right
            const dateWidth = doc.fontSize(9.5).font("Helvetica-Bold").widthOfString(dateText)
            doc.fontSize(9.5).font("Helvetica-Bold").fillColor("#000000")
               .text(mainText, leftMargin, doc.y, { width: pageWidth - dateWidth - 10 })

            // Right-align the date on the same line
            doc.fontSize(9.5).font("Helvetica-Bold")
               .text(dateText, leftMargin + pageWidth - dateWidth, doc.y - doc.currentLineHeight(), { width: dateWidth, align: "right" })
        } else if (line.match(/^(Bachelor|Master|B\.|M\.|Ph\.?D|Diploma|Associate|Doctor|CGPA|GPA)/i)) {
            // Degree line or CGPA
            const cgpaMatch = line.match(/(CGPA|GPA)\s*:?\s*[\d.]+/i)
            if (cgpaMatch) {
                const cgpaText = cgpaMatch[0]
                const degreeText = line.replace(cgpaText, "").trim()
                const cgpaWidth = doc.fontSize(9.5).font("Helvetica").widthOfString(cgpaText)
                doc.fontSize(9.5).font("Helvetica").fillColor("#000000")
                   .text(degreeText, leftMargin, doc.y, { width: pageWidth - cgpaWidth - 10 })
                doc.fontSize(9.5).font("Helvetica-Bold")
                   .text(cgpaText, leftMargin + pageWidth - cgpaWidth, doc.y - doc.currentLineHeight(), { width: cgpaWidth, align: "right" })
            } else {
                doc.fontSize(9.5).font("Helvetica").fillColor("#000000")
                   .text(line, leftMargin)
            }
...
function renderExperienceSection(doc, lines, leftMargin, pageWidth) {
    let i = 0
    while (i < lines.length) {
        const line = lines[i]
        ...
        if (isBullet) {
            ...
            renderBulletItem(doc, bulletText, leftMargin, pageWidth)
        } else {
            ...
            if (dateMatch) {
                doc.moveDown(0.2)
                const dateText = dateMatch[0]
                const mainText = line.replace(dateText, "").replace(/\s*[|,]\s*$/, "").trim()
                const dateWidth = doc.fontSize(9.5).font("Helvetica-Bold").widthOfString(dateText)
                ...
                doc.fontSize(9.5).font("Helvetica-Bold")
                   .text(dateText, leftMargin + pageWidth - dateWidth, doc.y - doc.currentLineHeight(), { width: dateWidth, align: "right" })
            } else if (line.match(/engineer|developer|designer|manager|analyst|intern|lead|architect|consultant/i)) {
                // Job title line
                doc.fontSize(9.5).font("Helvetica").fillColor("#000000")
                   .text(line, leftMargin)
            } else {
                // Company name or other bold line
                doc.moveDown(0.15)
                doc.fontSize(9.5).font("Helvetica-Bold").fillColor("#000000")
                   .text(line, leftMargin)
            }
...
function renderProjectsSection(doc, lines, leftMargin, pageWidth) {
    let i = 0
    while (i < lines.length) {
        const line = lines[i]
        checkPageBreak(doc, 30)
        ...
        if (isBullet) {
            ...
            const hasLink = /\blink\b/i.test(line)
            const titleText = line.replace(/\s*link\s*$/i, "").trim()

            if (hasLink) {
                const linkWidth = doc.fontSize(9.5).font("Helvetica-Bold").widthOfString("Link")
                doc.fontSize(9.5).font("Helvetica-Bold").fillColor("#000000")
                   .text(titleText, leftMargin, doc.y, { width: pageWidth - linkWidth - 10 })
                doc.fontSize(9.5).font("Helvetica-Bold")
                   .text("Link", leftMargin + pageWidth - linkWidth, doc.y - doc.currentLineHeight(), { width: linkWidth, align: "right" })
            } else {
                doc.fontSize(9.5).font("Helvetica-Bold").fillColor("#000000")
                   .text(titleText, leftMargin)
            }
        }
        i++
    }
...
function renderCertificationsSection(doc, lines, leftMargin, pageWidth) {
    let i = 0
    while (i < lines.length) {
        const line = lines[i]
        checkPageBreak(doc, 30)
...
function renderBulletList(doc, lines, leftMargin, pageWidth) {
    for (const line of lines) {
        checkPageBreak(doc, 20)
        const isBullet = /^[•●○▪▸\-\*]/.test(line) || /^[\d]+[\.\)]/.test(line)
        if (isBullet) {
            ...
            doc.fontSize(9.5).font("Helvetica").fillColor("#000000")
               .text(line, leftMargin, doc.y, { width: pageWidth })
        }
    }
    doc.moveDown(0.3)
}

function renderBulletItem(doc, text, leftMargin, pageWidth) {
    ...
    // Move back up to the same line and draw text
    doc.moveUp()
    doc.fontSize(9.5).font("Helvetica").fillColor("#000000")
       .text(text, leftMargin + bulletIndent + 5, doc.y, { width: pageWidth - bulletIndent - 5, lineGap: 2 })
}
