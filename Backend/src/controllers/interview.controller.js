...
async function generateInterviewReportController(req, res) {
    try {
        ...
        if (resumeFile) {
            ...
            // Isolate PDF parsing so a bad/corrupt/unsupported file
            ...
            try {
                ...
                if (!resumeContent || !resumeContent.trim()) {
                    console.warn("PDF parsed but no text content was extracted (possibly a scanned/image-only PDF)")
                    return res.status(400).json({
                        message: "Could not extract text from the uploaded resume. It may be a scanned image or corrupted PDF. Please try a different file or use the self-description field instead."
                    })
                }
            } catch (pdfError) {
                console.error("PDF parsing error:", pdfError)
                return res.status(400).json({
                    message: "Failed to read the uploaded resume file. Please make sure it is a valid, non-password-protected PDF, or use the self-description field instead.",
